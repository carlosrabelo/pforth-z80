...
test_entry:
    ...
    ; Assert HL == FROM_R_CFA
    ld iy, err_from_r_cfa_mismatch
    ld de, FROM_R_CFA
    ld l, c
    ld h, b
    call assert_de_hl
...
test_verify_code:
    ...
    ; 4. Verify that SP (RSP) is saved_rsp + 2 (value popped from return stack)
    ld hl, 0
    add hl, sp
    ld de, (saved_rsp)
    inc de
...
err_from_r_not_found:   db "Word 'R>' not found", 0
err_from_r_cfa_mismatch:  db "R> CFA mismatch", 0
...
err_rsp_mismatch:       db "RSP (SP) did not advance correctly after R>", 0
