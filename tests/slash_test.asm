; =============================================================================
; pForth - Z80 / (DIVIDE) Primitive Test
; =============================================================================
...
test_entry:
    ...
    ; Assert HL == SLASH_CFA
    ld iy, err_slash_cfa_mismatch
    ld de, SLASH_CFA
    ld l, c
    ld h, b
    call assert_de_hl

    ; --- TEST 2: Execute / with 7 / 3 (both positive) ---
    ...
    ; 5. Set up IP to run / and then verify_pos_pos
...
verify_pos_pos:
    ...
    ; --- TEST 3: Execute / with -7 / 3 (negative dividend) ---
    ...
    ; 4. Set up IP to run / and then verify_neg_pos
...
verify_neg_pos:
    ...
    ; --- TEST 4: Execute / with 7 / -3 (negative divisor) ---
    ; 1. Push anchor $9999 onto data stack
    dec ix
    ld (ix+0), $99          ; high byte of $9999
    dec ix
    ...
    ; 4. Set up IP to run / and then verify_pos_neg
...
verify_pos_neg:
    ...
    ; --- TEST 5: Execute / with -7 / -3 (both negative) ---
    ; 1. Push anchor $9999 onto data stack
    dec ix
    ld (ix+0), $99          ; high byte of $9999
    dec ix
    ...
    ; 4. Set up IP to run / and then verify_neg_neg
...
verify_neg_neg:
    ...
    ; 4. Set up IP to run / and then verify_zero_dividend
...
err_slash_not_found:                      db "Word '/' not found", 0
err_slash_cfa_mismatch:                   db "/ CFA mismatch", 0
err_pos_pos_quot_incorrect:               db "Quotient for 7 / 3 is incorrect (expected 2)", 0
err_neg_pos_quot_incorrect:               db "Quotient for -7 / 3 is incorrect (expected -2)", 0
err_pos_neg_quot_incorrect:               db "Quotient for 7 / -3 is incorrect (expected -2)", 0
err_neg_neg_quot_incorrect:               db "Quotient for -7 / -3 is incorrect (expected 2)", 0
err_zero_div_quot_incorrect:              db "Quotient for 0 / 5 is incorrect (expected 0)", 0
